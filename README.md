...
To do this, you just need to tell the Grunt task which directory (or directories) it should look in for your custom templates through the `includes` task option.
...
Let's say we'd like to always render a placeholder in our `<textarea>` elements in our rendered settings HTML.
...
Now we just need to tell our Grunt task to look in this directory for templates with the `includes` option:
...
In the example above, the first field will be render using our custom `placeholder-text-multi.html` template, while the second will be rendered with the default `text-multi.html` template. 
